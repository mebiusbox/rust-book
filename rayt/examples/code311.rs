...
impl ImageTexture {
    fn new(path: &str) -> Self {
        ...
        for (i, (_, _, pixel)) in image
            .iter_mut()
            .zip(rgbimg.enumerate_pixels()) {
...
impl Material for Dielectric {
    fn scatter(&self, ray: &Ray, hit: &HitInfo) -> Option<ScatterInfo> {
        ...
        if let Some(refracted) = (-ray.direction).refract(
            outward_normal, ni_over_nt) {
            if Vec3::random_full().x() > Self::schlick(cosine, self.ri) {
                return Some(ScatterInfo::new(
                    Ray::new(hit.p, refracted), Color::one(), None));
            }
        }
...
impl HitInfo {
    fn new(t: f64, p: Point3, n: Vec3, m: Arc<dyn Material>, u: f64, v: f64) 
        -> Self {
        Self { t, p, n, m, u, v }
    }
}
...
impl Shape for Sphere {
        ...
        if d > 0.0 {
            ...
            if t0 < temp && temp < t1 {
                ...
                return Some(HitInfo::new(
                    temp, p, n, Arc::clone(&self.material), u, v));
            }
            let temp = (-b + root) / (2.0 * a);
            if t0 < temp && temp < t1 {
                let p = ray.at(temp);
    ...
    fn random(&self, o: Vec3) -> Vec3 {
        ...
        ONB::new(direction).local(Vec3::random_to_sphere(
            self.radius, distance_squared))
    }
}
...
impl Rect {
    fn new(x0: f64, x1: f64, y0: f64, y1: f64, k: f64, axis: RectAxisType,
...
impl Shape for Rect {
    fn hit(&self, ray: &Ray, t0: f64, t1: f64) -> Option<HitInfo> {
        ...
        Some(HitInfo::new(
            t, ray.at(t), axis, Arc::clone(&self.material),
...
impl Shape for Rotate {
    fn hit(&self, ray: &Ray, t0: f64, t1: f64) -> Option<HitInfo> {
        ...
        let rotated_ray = Ray::new(
            revq.rotate(ray.origin), revq.rotate(ray.direction));
        if let Some(hit) = self.shape.hit(&rotated_ray, t0, t1) {
            Some(HitInfo {
                p: self.quat.rotate(hit.p), n: self.quat.rotate(hit.n), ..hit })
        } else {
            None
        }
    }
...
impl Shape for ShapeList {
    ...
    fn random(&self, o: Vec3) -> Vec3 {
        ...
        let rx = Vec3::random_full().x();
        let index = (rx * self.objects.len() as f64).floor() as usize;
...
impl ShapeBuilder {
    ...
    fn checker_texture(mut self, odd_color: Color, even_color: Color, freq: f64) 
        -> Self {
    ...
    fn rect_xy(mut self, x0: f64, x1: f64, y0: f64, y1: f64, k: f64) -> Self {
        self.shape = Some(Box::new(Rect::new(
            x0, x1, y0, y1, k, RectAxisType::XY, self.material.unwrap())));
        self.material = None;
        self
    }
    ...
    fn rect_xz(mut self, x0: f64, x1: f64, y0: f64, y1: f64, k: f64) -> Self {
        self.shape = Some(Box::new(Rect::new(
            x0, x1, y0, y1, k, RectAxisType::XZ, self.material.unwrap())));
        self.material = None;
        self
    }
    ...
    fn rect_yz(mut self, x0: f64, x1: f64, y0: f64, y1: f64, k: f64) -> Self {
        self.shape = Some(Box::new(Rect::new(
            x0, x1, y0, y1, k, RectAxisType::YZ, self.material.unwrap())));
        self.material = None;
        self
    }
    ...
    fn translate(mut self, offset: Point3) -> Self {
        self.shape = Some(Box::new(Translate::new(
            self.shape.unwrap(), offset)));
        self
    }

    fn rotate(mut self, axis: Vec3, angle: f64) -> Self {
        self.shape = Some(Box::new(Rotate::new(
            self.shape.unwrap(), axis, angle)));
        self
    }
...
impl SceneWithDepth for CornelBoxScene {
    ...
    fn width(&self) -> u32 { IMAGE_SIZE }   // IMAGE_SIZE は lib.rs で定義されています
    fn height(&self) -> u32 { IMAGE_SIZE }  // IMAGE_SIZE = IMAGE_WIDTH = 200
    fn spp(&self) -> usize { SPP }          // SPP は lib.rs で定義されています．SPP = 8
}
