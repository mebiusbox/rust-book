...
pub fn draw_in_window(backup_filename: &str, pixels: RgbImage) -> 
    minifb::Result<()> {
    ...
    window.update_with_buffer(
        &buffer, image_width as usize, image_height as usize)?;
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        if window.is_key_pressed(Key::D, KeyRepeat::No) {
            if backup_buffer.is_none() {
                if let Ok(img) = image::open(backup_filename) {
                    ...
                    for (i, (_, _, pixel)) in buf.iter_mut().zip(
                        backup_image.enumerate_pixels()) {
                        *i = u32::from_be_bytes(
        ...
        window.update_with_buffer(
            current_buffer, image_width as usize, image_height as usize)?;
    }
